...
class OnnxStableDiffusionInstructPix2PixPipeline(OnnxStableDiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        image_latents = self.prepare_image_latents(
            ...
            batch_size,
            num_images_per_prompt,
            torch.from_numpy(prompt_embeds).dtype,
            device,
            do_classifier_free_guidance,
            generator,
        )
        ...
        if num_channels_latents + num_channels_image != self.unet_in_channels:
            raise ValueError(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                scaled_latent_model_input = scaled_latent_model_input.numpy()
    ...
    def _encode_prompt(self, prompt, num_images_per_prompt, do_classifier_free_guidance, negative_prompt):
        ...
        prompt_embeds = self.text_encoder(input_ids=text_input_ids.numpy().astype(np.int32))[0]
       ...
       # get unconditional embeddings for classifier free guidance
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            negative_prompt_embeds = self.text_encoder(
                input_ids=uncond_input.input_ids.numpy().astype(np.int32)
            )[0]
        ...
        if do_classifier_free_guidance:
            ...
            prompt_embeds = torch.cat([prompt_embeds, negative_prompt_embeds, negative_prompt_embeds]).numpy()
    ...
    def prepare_image_latents(
        ...
        if isinstance(generator, list):
            ...
            image_latents = torch.cat(image_latents, dim=0).numpy()
        else:
        ...
        #image_latents = np.concatenate([image_latents] * batch_size, axis=0)
        #image_latents_orig = image_latents
